...
void sos_dma_unpin(void *cookie, void *addr, size_t size) { /* no op */
}

typedef int (*sel4_cache_op_fn_t)(seL4_ARM_PageDirectory, seL4_Word, seL4_Word);

static void cache_foreach(void *vaddr, int range, sel4_cache_op_fn_t proc) {
	int error;
	uintptr_t next;
...
void sos_dma_cache_op(void *cookie, void *addr, size_t size,
					  dma_cache_op_t op) {
