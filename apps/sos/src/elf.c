...
static int load_segment_into_vspace(seL4_ARM_PageDirectory dest_as, char *src,
	...
	*/
	...
	while (pos < segment_size) {
		...
		err = cspace_ut_retype_addr(paddr, seL4_ARM_SmallPageObject,
									seL4_PageBits, cur_cspace, &tty_cap);
		conditional_panic(err, "Failed to retype to a frame object");

		/* Copy the frame cap as we need to map it into 2 address spaces */
		sos_cap =
			cspace_copy_cap(cur_cspace, cur_cspace, tty_cap, seL4_AllRights);
...
int elf_load(seL4_ARM_PageDirectory dest_as, char *elf_file) {
	...
	for (i = 0; i < num_headers; i++) {
		...
		err = load_segment_into_vspace(
			dest_as, source_addr, segment_size, file_size, vaddr,
