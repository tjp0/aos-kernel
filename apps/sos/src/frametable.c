...
int frame_physical_alloc(void** vaddr) {
    ...
    int err =  cspace_ut_retype_addr(new_frame_addr, 
    ...
    err = map_page(new_frame, 
           seL4_CapInitThreadPD, 
           FRAME_VSTART+new_frame_addr,
           seL4_AllRights,
           0);
...
seL4_Word ft_early_initialize(const seL4_BootInfo* info) {
	...
	for (int i = 0; i < info->untyped.end-info->untyped.start; i++) {
		uint32_t paddr = info->untypedPaddrList[i]+(1<<info->untypedSizeBitsList[i]);
	   if(paddr > max_paddr) {
	   	max_paddr = paddr;
	   }
    }
...
void ft_late_initialize(seL4_Word base) {
	...
	{
		seL4_Word new_frame;
		conditional_panic(FRAME_TABLE_VSTART+offset >= FRAME_TABLE_VEND, "Frame table too big for allocated memory map");

	    int err =  cspace_ut_retype_addr(base+offset, 
	                             seL4_ARM_SmallPageObject,
	                             seL4_PageBits,
	                             cur_cspace,
	                             &new_frame);
