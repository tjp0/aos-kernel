 ...
 * @return 0 on success
 */
static int _map_page_table(seL4_ARM_PageDirectory pd, seL4_Word vaddr) {
	seL4_Word pt_addr;
	...
	err = seL4_ARM_PageTable_Map(pt_cap, pd, vaddr,
...
void* map_device(void* paddr, int size) {
	...
	while (virt - vstart < size) {
		...
		/* Map in the page */
		err =
			map_page(frame_cap, seL4_CapInitThreadPD, virt, seL4_AllRights, 0);
