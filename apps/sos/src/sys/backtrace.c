...
static void init(void) {
	...
	if (libgcc_handle == NULL) return;

	unwind_backtrace = __libc_dlsym(libgcc_handle, "_Unwind_Backtrace");
	unwind_vrs_get = __libc_dlsym(libgcc_handle, "_Unwind_VRS_Get");
	if (unwind_vrs_get == NULL) unwind_backtrace = NULL;
}
...
static _Unwind_Reason_Code backtrace_helper(struct _Unwind_Context *ctx,
	...
	if (arg->cnt != -1) arg->array[arg->cnt] = (void *)unwind_getip(ctx);
	if (++arg->cnt == arg->size) {
...
int backtrace(void **array, int size) {
	...
	if (unwind_backtrace == NULL) return 0;
#endif

	if (size >= 1) unwind_backtrace(backtrace_helper, &arg);

	if (arg.cnt > 1 && arg.array[arg.cnt - 1] == NULL) --arg.cnt;
