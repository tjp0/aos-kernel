...
static int __ut_cmp(const void* _a, const void* _b) {
	return _bi->untypedPaddrList[*(int*)_a] - _bi->untypedPaddrList[*(int*)_b];
}
...
static inline void _print_table(void) {
	...
	for (i = 0; i < _ut.count; i++) {
		dprintf(0, "| %2d(%2d) | 0x%08x -> 0x%08x | %9d |\n", i, _ut.map[i],
				UT_PSTART(i), UT_PEND(i), UT_SIZEBITS(i));
...
int ut_translate(seL4_Word addr, seL4_Untyped* ret_cptr,
				 seL4_Word* ret_offset) {
	int start, end;
