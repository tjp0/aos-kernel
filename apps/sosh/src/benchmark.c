...
_Static_assert(BIT(MAX_BUF_SIZE) <= TOTAL_FILE_SIZE,
			   "total file size must be a multiple of MAX_BUF_SIZE");
_Static_assert(MIN_BUF_SIZE > 0, "min buf size bigger than 0");
_Static_assert(MAX_BUF_SIZE >= MIN_BUF_SIZE,
			   "min buf size smaller than or eq to max buf size");
...
static void init_ccnt(void *arg) {
	...
	asm volatile(
		...
		: /* no outputs */
		: "r"(1), "r"(-1), "r"(pmcr | CCNT_RESET | CCNT_ENABLE | CCNT_64),
		  "r"(PMCNTENSET)
		: /* no clobber */
		);
}
...
static int run_benchmark(char *name, benchmark_fn_t fn, uint32_t overhead,
	 ...
	 * 512b, 4k, 32k, 256k */
	for (size_t sz = BIT(MIN_BUF_SIZE); sz <= BIT(MAX_BUF_SIZE);
		 sz = sz << 3u) {
		uint32_t start, end;
		...
		for (int i = 0; i < N_RESULTS; i++) {
			...
			for (int j = 0; j < LOOPS; j++) {
				...
				if (debug_mode) {
					...
					if (res != sz) {
						printf(
							"benchmark_fn did not %s full buf size, only "
							"%u/%u\n",
							name, res, sz);
						sos_sys_close(fd);
						return -1;
 ...
 * 'Build Options' -> 'Build kernel with support for executing arbitrary code in
 * protected mode'
