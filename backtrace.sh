...
BACKTRACE_PC = "" LEVEL = 0 OBJDUMP_OPTIONS = "-d" WIDTH = 4 COMPILER_PREFIX =
	arm - linux - gnueabi - SOS_BIN_PATH = "build/arm/imx6/sos/sos.bin"

	NM_EXE = nm OBJDUMP_EXE = objdump NM = $COMPILER_PREFIX$NM_EXE OBJDUMP =
		$COMPILER_PREFIX$OBJDUMP_EXE
#help function for the backtrace_symbol script
		print_help() {
	echo "USAGE:    backtrace.sh [OPTIONS] <<EOF" echo
		 "           <pc_addr0> <pc_addr1> <pc_addr2> <pc_addr3> ..." echo
		 "           <<EOF" echo echo
		 "OPTIONS:  -l printout line number for disassembly" echo
		 "          -S printout source code for disassembly" echo
		 "          -w WIDTH extra lines of disassembly for each side of the "
		 "target PC, between 0 and 32 (default is 4)" echo
		 "          --start-address=0xADDR only process data whose address is "
		 ">= ADDR" echo
		 "          --stop-address=0xADDR  only process data whose address is "
		 "<= ADDR" echo "          --help printout this message" echo echo
		 "Format:   The PCs(Program Counter Values) should be space or newline "
		 "separated" echo
		 "          hex numbers, starting with \"0x\"" exit 0 return
}
...
#resolve the line number surround each pc, [0 - 10]
								  if[$2 - lt 33] &&
								  [$2 - gt - 1];
	then WIDTH = $2 shift else
#call the help function
		print_help fi

	elif [ `expr "$1":'--start-address=0x'` - eq 18] then
#passing start address address to the objdump
		OBJDUMP_OPTIONS = "$OBJDUMP_OPTIONS $1"

	elif [ `expr "$1":'--stop-address=0x'` - eq 17] then
...
#the first string in each line is an 8 digits hex number
	if (length($1) != 8)
		next
			...
			a1 = ("0x" $1) + 0 #numeric the string if (a1 < target) {
			pervious_name = $3 pervious_add = a1
		}
	else if (a1 == target) {
#printout the name in this line
#target find at the start of the function
		resolve = 1 printf "    #%2d  0x%08x in %-30s\n", le, target, $3 exit 1
	} else {
#printout the pervious recorded name
		resolve = 1 printf "    #%2d  0x%08x in  %-30s\n", le, target,
		pervious_name exit 1
	}
}
END {
	if (resolve == 0) {
...
#printout the relative disassembled code if the pc is resolved
	EXIT_STATUS =
		$ ? if[$EXIT_STATUS - eq 1];
then echo "" awk - v searchadd = $ADDRESS - v width = $WIDTH \
            ' BEGIN { add = searchadd + 0 #numeric the search address start =
								 add - width* 4 end = add + width* 4 flag = 0
}
{
...
#the first line should be the address of the pc, in the form hex:
		if (length($1) > 9)
			next
				...
				a1 = ("0x" $1) + 0 #numeric the pc string if (a1 == end) {
				print $0 exit #print finish
			}
