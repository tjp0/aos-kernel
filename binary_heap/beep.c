...
int is_empty(beep_struct *beep) { return beep->nodes_in_use == 0; }

int is_full(beep_struct *beep) {
	return beep->nodes_in_use == beep->node_capacity;
}

int num_nodes(beep_struct *beep) { return beep->nodes_in_use; }
...
int node_capacity(beep_struct *beep) { return beep->node_capacity; }
...
void percolate_down(beep_struct *beep, unsigned int pos) {
	...
	while (1) {
		...
		if (next_pri < percolate_pri) {
			...
			set_node_from_node(&(beep->node_array[cur_pos - 1]),
