...
static void init_timer_list(void){
	first_free = (timer_struct *)malloc(sizeof(timer_struct) * TIMER_INITIAL_CAPACITY);
...
static int expand(void){
	...
	beep_struct* realloced = realloc(beep, get_beep_size_for_n_nodes(new_capacity));
	if(realloced == NULL) {
		if(DEBUG) {
	...
	timer_struct* temp = realloc(timer_list, sizeof(timer_struct) * new_capacity);
	if(temp == NULL) {
		if(DEBUG) {
			dprintf(0,"Failed to expand\n");
		}
...
int timer_interrupt(void){
	...
	while(peek(beep) != -1 && timer_from_id(peek(beep))->delay <= time_stamp())
	{
		...
		/* Bugfix: If the timer list is expanded during the callback, this pointer may not be valid */
