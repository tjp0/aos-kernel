...
struct mib_regs {
	/* NOTE: Counter not implemented because it is not applicable (read 0
	 * always).*/
	uint32_t
		rmon_t_drop; /* 00 Register Count of frames not counted correctly */
	...
	/* NOTE: Counter not implemented because there is no SQE information
	 * available (read 0 always). */
	...
	uint32_t
		ieee_t_octets_ok; /* 74 Octet count for Frames Transmitted w/o Error */
	uint32_t res0[3];
...
#define TCR_TFCPAUSE \
	BIT(3) /* Transmit pause frame after the current frame completes */
...
#define PAUSE_OPCODE_FIELD \
	0x0001 /* Fixed magic opcode used when sending pause frames */
/* configurable */
#define PAUSE_DURATION 32 /* Pause duration field when sending pause frames */
#define STRFWD_BYTES \
	128 /* Number of bytes in buffer before transmission begins */
...
static void _mdc_clk_recal(struct clock* clk) { assert(0); }

static clk_t* _mdc_clk_init(clk_t* clk) { return clk; }
...
uint32_t enet_get_events(struct enet* enet) { return enet_get_regs(enet)->eir; }
