...
#define RXD_ERROR \
	(RXD_BADLEN | RXD_BADALIGN | RXD_CRCERR | RXD_OVERRUN | RXD_TRUNC)
...
static void fill_rx_bufs(struct eth_driver *driver) {
	...
	while (dev->rx_remain > 0) {
		...
		uintptr_t phys =
			driver->i_cb.allocate_rx_buf(driver->cb_cookie, BUF_SIZE, &cookie);
		if (!phys) {
			break;
		}
		...
		__sync_synchronize();
		dev->rx_ring[dev->rdt].stat =
			RXD_EMPTY | (next_rdt == 0 ? RXD_WRAP : 0);
		dev->rdt = next_rdt;
		dev->rx_remain--;
	}
	__sync_synchronize();
...
static void free_desc_ring(struct imx6_eth_data *dev, ps_dma_man_t *dma_man) {
	if (dev->rx_ring) {
		dma_unpin_free(dma_man, (void *)dev->rx_ring,
					   sizeof(struct descriptor) * dev->rx_size);
		dev->rx_ring = NULL;
	}
	if (dev->tx_ring) {
		dma_unpin_free(dma_man, (void *)dev->tx_ring,
...
static int initialize_desc_ring(struct imx6_eth_data *dev,
	...
	dma_addr_t rx_ring = dma_alloc_pin(
		dma_man, sizeof(struct descriptor) * dev->rx_size, 0, DMA_ALIGN);
	if (!rx_ring.phys) {
	...
	dma_addr_t tx_ring = dma_alloc_pin(
		dma_man, sizeof(struct descriptor) * dev->tx_size, 0, DMA_ALIGN);
	if (!tx_ring.phys) {
		LOG_ERROR("Failed to allocate tx_ring");
		free_desc_ring(dev, dma_man);
		return -1;
	}
	ps_dma_cache_clean_invalidate(dma_man, rx_ring.virt,
								  sizeof(struct descriptor) * dev->rx_size);
	ps_dma_cache_clean_invalidate(dma_man, tx_ring.virt,
								  sizeof(struct descriptor) * dev->tx_size);
	...
	/* Remaining needs to be 2 less than size as we cannot actually enqueue size
	 * many descriptors,
...
static void print_state(struct eth_driver *eth_driver) {
	struct imx6_eth_data *eth_data =
		(struct imx6_eth_data *)eth_driver->eth_data;
...
static struct raw_iface_funcs iface_fns = {.raw_handleIRQ = handle_irq,
...
int ethif_imx6_init(struct eth_driver *eth_driver, ps_io_ops_t io_ops,
	...
	enet = enet_init((struct desc_data){.tx_phys = eth_data->tx_ring_phys,
