 ...
 * the combined responsibility of LWIP and the application to monitor and
 * process network traffic whilst waiting for a response to an NFS transaction.
 * When the response arrives, the registered callback for the transaction will
 * be called to complete the transaction. Data provided to the call back
 * functions are available only during the execution of the call back function.
 * It is the applications responsibility to copy this data to an alternate
 * location if this data is required beyond the scope of the callback.
 *
...
typedef enum nfs_stat {
    ...
    /// requested operation.
    NFSERR_PERM = 1,
    /// No such file or directory.  The file or directory specified does not
    /// exist.
    NFSERR_NOENT = 2,
    ...
    /// limit.
    NFSERR_FBIG = 27,
    /// No space left on device.  The operation caused the servers file system
    /// to
    /// reach its limit.
    NFSERR_NOSPC = 28,
    ...
    /// referred to by that file handle no longer exists, or access to it has
    /// been
    /// revoked.
    NFSERR_STALE = 70,
    /// The servers write cache used in the "WRITECACHE" call got flushed to
    /// disk.
    NFSERR_WFLUSH = 99,
 ...
 * that this does not always mean that the transaction was successfully
 * delivered.
 */
...
typedef struct fattr {
    ...
    uint32_t mode;
    /// The number of hard links to the file (the number of different names for
    /// the same file).
    uint32_t nlink;
    ...
    timeval_t mtime;
    /// The time when the status of the file was last changed. Writing to the
...
typedef struct sattr {
    ...
    /// The group identification number of the group of the file.
    uint32_t gid;
    /// The size in bytes of the file. Zero means that the file should be
    /// truncated.
    uint32_t size;
    /// The time when the file was last accessed for either read or write.
    timeval_t atime;
 ...
 *                     received.
 */
enum rpc_stat nfs_getattr(const fhandle_t *fh, nfs_getattr_cb_t callback,
                          uintptr_t token);
...
/** I NEED TO IMPLEMENT THIS
    * copy data from user space in tthe kernel
    * call nfswrite on this data
    * and the yield
    * in a lloop
...
enum rpc_stat nfs_write(const fhandle_t *fh, int offset, int count,
                        const void *data, nfs_write_cb_t callback,
                        uintptr_t token);
