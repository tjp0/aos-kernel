...
void nfs_timeout(void) { rpc_timeout(100); }
...
enum rpc_stat nfs_init(const struct ip_addr *server) {
    ...
    switch (port) {
        case -1:
            printf(
                "Communication error when acquiring NFS port from "
                "portmapper\n");
            return RPCERR_COMM;
        case -0:
        case -2:
            printf(
                "Error when acquiring NFS port number from portmapper: Not "
                "supported\n");
            return RPCERR_NOSUP;
        default:
...
static void _nfs_getattr_cb(void *callback, uintptr_t token,
                            struct pbuf *pbuf) {
...
enum rpc_stat nfs_getattr(const fhandle_t *fh, nfs_getattr_cb_t func,
                          uintptr_t token) {
    struct pbuf *pbuf;
    int pos;
...
static void _nfs_lookup_cb(void *callback, uintptr_t token, struct pbuf *pbuf) {
    uint32_t status = NFSERR_COMM;
    fhandle_t new_fh;
    fattr_t pattrs;
    struct rpc_reply_hdr hdr;
...
enum rpc_stat nfs_lookup(const fhandle_t *cwd, const char *name,
...
static void _nfs_read_cb(void *callback, uintptr_t token, struct pbuf *pbuf) {
    uint32_t status = NFSERR_COMM;
    fattr_t pattrs;
    char *data = NULL;
    uint32_t size = 0;
...
static void _nfs_write_cb(void *callback, uintptr_t token, struct pbuf *pbuf) {
...
enum rpc_stat nfs_write(const fhandle_t *fh, int offset, int count,
                        const void *data, nfs_write_cb_t func,
                        uintptr_t token) {
    struct pbuf *pbuf;
...
static void _nfs_create_cb(void *callback, uintptr_t token, struct pbuf *pbuf) {
    uint32_t status = NFSERR_COMM;
    fhandle_t new_fh;
    fattr_t pattrs;
    struct rpc_reply_hdr hdr;
...
enum rpc_stat nfs_create(const fhandle_t *fh, const char *name,
                         const sattr_t *sat, nfs_create_cb_t func,
                         uintptr_t token) {
    struct pbuf *pbuf;
    int pos;
...
void _nfs_remove_cb(void *callback, uintptr_t token, struct pbuf *pbuf) {
    uint32_t status = NFSERR_COMM;
    struct rpc_reply_hdr hdr;
    int pos;
...
enum rpc_stat nfs_remove(const fhandle_t *fh, const char *name,
...
static void _nfs_readdir_cb(void *callback, uintptr_t token,
                            struct pbuf *pbuf) {
    nfs_readdir_cb_t cb;
    int num_entries = 0;
    char **entries = NULL;
